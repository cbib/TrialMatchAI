...
def load_trial_data(json_folder: str) -> List[Dict]:
    ...
    for file_name in os.listdir(json_folder):
        if file_name.endswith('.json'):
            ...
            trial_id = os.path.splitext(file_name)[0]  # Extract TrialID from the file name
            try:
...
def score_trial(trial: Dict) -> float:
    ...
    """
    def calculate_ratio(criteria_list, positive_classifications, negative_classifications):
        criteria_to_exclude = ['Irrelevant', 'Unclear']
        criteria_list = [c for c in criteria_list if not c.get('Classification') in criteria_to_exclude]
        total_criteria = len(criteria_list)  # Adjusted total excludes 'Insufficient Information'
        if total_criteria == 0:
            return 0.0

        positive_count = sum(1 for c in criteria_list if c.get('Classification') in positive_classifications)
        negative_count = sum(1 for c in criteria_list if c.get('Classification') in negative_classifications)
...
def run_final_ranking(output_folder):
    ...
    from final_ranker import load_trial_data, rank_trials, save_ranked_trials
    trials = load_trial_data(output_folder)
    ranked_trials = rank_trials(trials)
    save_ranked_trials(ranked_trials, ranked_trials_path)
