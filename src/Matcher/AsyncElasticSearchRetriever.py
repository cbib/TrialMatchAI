...
class AsyncElasticsearchRetriever(BaseRetriever):
    """
    Asynchronous Elasticsearch retriever

    Args:
        es_client: AsyncElasticsearch client connection. Alternatively, you can use the
    ...
    def from_es_params(
        ...
        return AsyncElasticsearchRetriever(
            es_client=client,
    ...
    async def abatch(self, queries: List[str]) -> List[Dict[str, Any]]:
        tasks = [self._get_relevant_documents(query) for query in queries]
