...
class SecondLevelSentenceEmbedder:
    ...
    def __init__(self, model_name: str = 'BAAI/bge-m3', use_gpu: bool = True):
        ...
        self.model.eval()
        logger.info("SecondLevelSentenceEmbedder initialized with model: %s", model_name)
...
class QueryEmbedderSecondLevel:
    ...
    """
    def __init__(self, model_name="ncbi/MedCPT-Query-Encoder", max_length=10, use_gpu=True):
        self.device = torch.device("cuda" if use_gpu and torch.cuda.is_available() else "cpu")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        self.model = AutoModel.from_pretrained(model_name).to(self.device)
...
def transform_func(tokenizer: PreTrainedTokenizerFast, max_length: int, examples: Dict[str, List]) -> BatchEncoding:
    return tokenizer(examples['contents'], max_length=max_length, padding=True, return_token_type_ids=False, truncation=True)
    ...
    def _do_encode(self, input_texts: List[str]) -> np.ndarray:
        ...
        for batch_dict in tqdm(data_loader, desc='encoding', mininterval=10):
            ...
            with torch.amp.autocast('cuda'):
                ...
                if isinstance(outputs, torch.Tensor):  # If model outputs a tensor directly
...
class SecondStageRetriever:
    ...
    def retrieve_criteria(self, nct_ids: List[str], queries: List[str]) -> Dict[str, List[Dict]]:
        """
        ...
        def execute_query(query):
            ...
            hits = hits[:int(len(hits) * 1.0)]  # Top 95% filtering (effectively keeps all results)

            logger.info(f"Retrieved {len(hits)} documents for query: '{query}'")
            return query, hits

        # Use ThreadPoolExecutor to parallelize query execution
        with ThreadPoolExecutor(max_workers=min(8, len(queries))) as executor:  # Limit concurrency to 8 threads
    ...
    def rerank_criteria(self, queries: List[str], criteria: List[Dict]) -> List[Dict]:
        ...
        pairs = [(criterion['query'], criterion['_source']['criterion']) for criterion in criteria]
    ...
    def aggregate_to_trials(self,
        ...
        sorted_trials = [
            ...
            for nct_id, score in sorted(aggregated_scores.items(), key=lambda x: x[1], reverse=True)
        ]
        return sorted_trials

    def retrieve_and_rank(self, queries: List[str], nct_ids: List[str], top_n: int) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    retriever = SecondStageRetriever(es_client, llm_reranker_model, embedder, index_name, bio_med_ner=bio_med_ner)
