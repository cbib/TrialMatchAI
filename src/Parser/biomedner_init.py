...
from typing import Any, Callable, Dict, List, Optional, NewType, NamedTuple, Tuple
...
class BioMedNER:
    def __init__(self, params):
        ...
        if "roberta" in self.params.model_name_or_path or "biolm" in self.params.model_name_or_path:
            # roberta
    ...
    def _prediction_loop(
    ...
    ) -> Tuple[PredictionOutput, PredictionOutput, PredictionOutput, PredictionOutput, PredictionOutput, PredictionOutput, PredictionOutput, PredictionOutput]:
        """
        ...
        dise_preds = chem_preds = gene_preds = spec_preds = cl_preds = dna_preds = rna_preds = ct_preds = None
        label_ids = None
        model.eval()
            ...
            for inputs in tqdm(dataloader, desc=description):
                ...
                if has_labels and not prediction_loss_only:
                    ...
                    if label_ids is None:
                        ...
                        label_ids = torch.cat((label_ids, inputs["labels"].detach().clone()), dim=0)

        # Convert all prediction tensors to numpy arrays
        if all(pred is not None for pred in [dise_preds, chem_preds, gene_preds, spec_preds, cl_preds, dna_preds, rna_preds, ct_preds]):
